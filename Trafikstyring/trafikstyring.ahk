...
(
    ...
    SvigtÅbningstidUdskudtKnapTjekVar := 0
        ...
        ; Opbyg garantivognssdata
        ; TODO omdøb variabler
        gv_svigt := []
        FileRead, gv_svigt_ind, db\gv_svigt.txt
        gv_svigt_ind := StrReplace(gv_svigt_ind, "`r", "")
        gv_svigt_ind := StrSplit(gv_svigt_ind, "`n")
        ...
        ; tjek ugedag op mod garantitidsdage
        dato := vl_array.4
    ...
    if (SvigtVlLukketRadio = 1 and SvigtVlSlettetRadio = 1)
    {
        sleep 100
    ...
    if (SvigtVlLukketRadio = 1 and StrLen(GVHjemzoneTid) != 4)
    {
        sleep 100
    ...
    if (vlTypeGaranti = 1 and SvigtVlLukketRadio = 1 and årsag != "")
    {
    ...
    if (vlTypeGaranti = 1 and SvigtVlSlettetRadio = 1 and årsag = "")
        ...
        if (SvigtVmKontaktradio = 1)
            {
                ...
                mail_indhold.broedtekst := "Vl slettet. Garantitid " SvigtGarantiTidTekstMail ". VM kontaktet cirka " svigtVmKontaktEdit " — " . svigtBeskrivelse
                    gui, hide
                    return mail_indhold
                }
        if (SvigtIngenVmKontaktradio = 1)
        ...
        if (vlTypeGarantiVariabel = 1 and SvigtVlLukketRadio = 0 and SvigtVlSlettetRadio = 0 and årsag != "")
        {
            ...
            if (SvigtVmKontaktradio = 1)
                {
                    ...
                    mail_indhold.broedtekst := "GV slettet ifm. variabel kørsel. Garantitid: " SvigtGarantiTidTekstMail ". VM kontaktet cirka " svigtVmKontaktEdit " — " . svigtBeskrivelse
                    gui, hide
                    return mail_indhold
                }
            if (SvigtIngenVmKontaktradio = 1)
                {
                    mail_indhold.emnefelt := "Svigt VL " SvigtVlEdit " " vl_type ": ikke startet op d. " dato
                    ; MsgBox, , 7.1, % mail_indhold.emnefelt,
                    mail_indhold.broedtekst := "GV slettet ifm. variabel kørsel. Garantitid: " SvigtGarantiTidTekstMail ". VM forgæves forsøgt kontaktet cirka " svigtVmKontaktEdit " — " . svigtBeskrivelse
                    gui, hide
                    return mail_indhold
                }
        ...
        if (vlTypeGarantiVariabel = 1 and SvigtVlLukketRadio = 1 and årsag != "")
        {
            ...
            if (tid_slet != "Åbningstid garanti")
                mail_indhold.broedtekst := "Variabel kørsel, lukket kl. " GVHjemzoneTid ". Garantitid: " SvigtGarantiTidTekstMail " — " . svigtBeskrivelse
            Else
    ...
    gui_svigt_vis(mail_indhold, skærmprint, GemtSkærmprint, gemtklip, VGPrint)
    {
       ...
       outlook := ComObjCreate("Outlook.application")
        ...
        broedtekst := mail_indhold.broedtekst
        html_tekst =
        (
            <META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=iso-8859-1">
    <html xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns:m="http://schemas.microsoft.com/office/2004/12/omml" xmlns="http://www.w3.org/TR/REC-html40"><head><meta name=Generator content="Microsoft Word 15 (filtered medium)"><!--[if !mso]><style>v\:* {behavior:url(#default#VML);}
